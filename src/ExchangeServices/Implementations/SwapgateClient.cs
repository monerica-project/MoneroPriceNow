...
public sealed class SwapgateClient : ISwapgateClient
{
    ...
    private readonly SwapgateOptions _opt;
    ...
    {
        var probe = _opt.SellProbeAmountXmr;

        var (amountToGet, minRequired) = await GetRateWithMinAsync(
            fromCurrency: _opt.XmrCurrency,
            fromNetwork: _opt.XmrNetwork,
            toCurrency: _opt.UsdtCurrency,
            toNetwork: _opt.UsdtNetwork,
            depositAmount: probe,
            ct);
    ...
    public async Task<PriceResult?> GetBuyPriceAsync(PriceQuery query, CancellationToken ct = default)
    {
        ...
        var (amountToGet, minRequired) = await GetRateWithMinAsync(
            fromCurrency: _opt.UsdtCurrency,
            fromNetwork: _opt.UsdtNetwork,
            toCurrency: _opt.XmrCurrency,
            toNetwork: _opt.XmrNetwork,
            depositAmount: probe,
            ct);
