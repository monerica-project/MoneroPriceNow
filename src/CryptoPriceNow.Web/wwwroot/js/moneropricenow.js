(() => {
  ...
  function renderHero(s) {
    ...
    heroAvgBuyN.textContent  = s.avgBuyCount
      ? `${s.avgBuyCount} exchange${s.avgBuyCount  !== 1 ? 's' : ''} · you pay`
      : 'you pay per XMR';

    // Right: avg sell
    heroAvgSellEl.textContent = s.avgSellVal != null ? fmt(s.avgSellVal) : '—';
    heroAvgSellN.textContent  = s.avgSellCount
      ? `${s.avgSellCount} exchange${s.avgSellCount !== 1 ? 's' : ''} · you receive`
      : 'you receive per XMR';
  }
