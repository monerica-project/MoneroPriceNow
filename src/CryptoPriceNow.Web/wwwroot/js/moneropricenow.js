(() => {
    ...
    async function loadSponsors() {
        try {
            ...
            bodyEl.querySelectorAll('tr[data-ex]').forEach(tr => {
                ...
                if (row) {
                    applySponsorClass(tr, row.siteName);
                    // Also swap the name cell link now that sponsor URLs are known
                    if (isSponsor(row.siteName)) {
                        tr.querySelector('td:first-child').innerHTML = nameLink(row);
                    }
                }
            });
        } catch (e) {
    ...
    function recordPrice(mid) {
        ...
        // Session tracker: lock in the first price seen this visit
        if (sessionStartPrice === null) {
            sessionStartPrice = mid;
            sessionStartTime = Date.now();
        }
        updateSessionDelta(mid);
    ...
    function fmtElapsed(ms) {
        ...
        if (s < 60) return s + 's on page';
        const m = Math.floor(s / 60), rem = s % 60;
        if (m < 60) return m + 'm ' + rem + 's on page';
        const h = Math.floor(m / 60), remM = m % 60;
        return h + 'h ' + remM + 'm on page';
    }

    function updateSessionDelta(mid) {
        if (!sessionDeltaEl || sessionStartPrice == null) return;
        const pct = ((mid - sessionStartPrice) / sessionStartPrice) * 100;
        const abs = Math.abs(pct);
        const sign = pct > 0 ? '+' : pct < 0 ? '−' : '';
        sessionDeltaEl.textContent = sign + abs.toFixed(3) + '%';
        sessionDeltaEl.className = 'session-delta' +
            (pct > 0.0001 ? ' sd-up' : pct < -0.0001 ? ' sd-down' : '');
        if (sessionStartTime != null) {
            sessionDeltaEl.dataset.elapsed = fmtElapsed(Date.now() - sessionStartTime);
        }
    }
    ...
    function mapRow(x) {
        ...
        return {
            ...
            minAmountUsd: num(pick(x, 'minAmountUsd', 'MinAmountUsd', 'minAmount', 'MinAmount'))
        };
    }
    ...
    function sortedRows(rows) {
        ...
        return [...rows].sort((a, b) => {
            ...
            if (sortKey === 'name') {
                ...
                v = av - bv;
            } else if (sortKey === 'minamt') {
                const av = a.minAmountUsd ?? (sortDir > 0 ? Infinity : -Infinity);
                const bv = b.minAmountUsd ?? (sortDir > 0 ? Infinity : -Infinity);
                v = av - bv;
            }
            return v * sortDir;
        });
    ...
    function renderTable(rows, bestBuy, worstBuy, bestSell, worstSell) {
        if (!rows.length) {
            bodyEl.innerHTML = `<tr><td colspan="7" style="padding:1.5rem 1rem;color:var(--text-muted);">No prices available.</td></tr>`;
        ...
        if (tableFirstRender) {
          ...
          <td data-col="minamount">${fmtMinAmt(r.minAmountUsd)}</td>
        ...
        sorted.forEach(r => {
            ...
            if (!tr) {
                ...
                tr.dataset.ex = key;
                tr.innerHTML = `<td>${nameLink(r)}</td><td data-col="privacy"></td><td data-col="buy"></td><td data-col="sell"></td><td data-col="spread"></td><td data-col="minamount"></td><td class="ts-cell" data-col="ts"></td>`;
            ...
            minAmountCell.textContent = fmtMinAmt(r.minAmountUsd);
    ...
    async function refresh() {
        try {
            ...
            if (s.midPrice != null) {
                window.__MARKET_PRICE__ = s.midPrice;
                if (window.__convRecalc) window.__convRecalc();
            }
...
(function initConverter() {
    ...
    function getMarketPrice() {
        ...
        if (hero) {
            const n = parseFloat(hero.textContent.replace(/[^0-9.]/g, ''));
    ...
    function recalc() {
        ...
        if (lastEdited === 'usd') {
            ...
            xmrInput.value = (!isNaN(usd) && usd > 0) ? fmtXmr(usd / price) : '';
        } else {
            const xmr = parseFloat(xmrInput.value);
            usdInput.value = (!isNaN(xmr) && xmr > 0) ? fmtUsd(xmr * price) : '';
        }
    }
